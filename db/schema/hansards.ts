...
import { date, pgTable, primaryKey, foreignKey, smallint, integer, varchar, boolean, text, customType, index } from 'drizzle-orm/pg-core';
...
export const proceedings = pgTable("proceedings", {
    ...
    fk_session: foreignKey({ columns: [t.legislature, t.session], foreignColumns: [sessions.legislature, sessions.session]})
}));
...
export const speeches = pgTable("speeches", {
    ...
    search: tsVector("search").generatedAlwaysAs((): SQL => sql`to_tsvector('french', ${speeches.text})`),
}, (t) => ({

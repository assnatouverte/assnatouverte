...
export async function getSessionsFromAssNat(): Promise<Session[]> {
    const mainPageReq = await http.get('http://www.assnat.qc.ca/fr/patrimoine/datesessions.html').text();
    const mainPage = $.load(mainPageReq);

    // Find table
    const rows = mainPage('.tableauLegisSessionsDepuis1867>tbody>tr:nth-of-type(n+3)').toArray();

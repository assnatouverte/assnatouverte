...
export async function downloadAllHansards(legislature: number, session: number, dir: string) {
    const sessionString = `${legislature}-${session}`;
    const url = `https://www.assnat.qc.ca/fr/travaux-parlementaires/assemblee-nationale/${sessionString}/index.html`;
    const monthChoiceField = 'ctl00$ColCentre$ContenuColonneGauche$ddlChoixMoisAnnee$MoisAnnee';

    // Load main page to list the months
    console.log(`Récupération de journaux de débats de la session ${sessionString}...`);
    const mainPageReq = await http.get(url).text();
    const mainPage = cheerio.load(mainPageReq);

    const months = mainPage(`select[name="${monthChoiceField}"]>option`).map((_, el) => el.attribs['value']).toArray();
    ...
    const tasks = months.map(async (month: string) => {
        ...
        // Mois courant
        const currentMonth = $('#ctl00_ColCentre_ContenuColonneGauche_lblMoisCourant').text();

        // Extrait tous les liens vers les journaux de débats
        const links = $('#ctl00_ColCentre_ContenuColonneGauche_pnlContenu a[href^="/fr/travaux-parlementaires/assemblee-nationale"]').map((_, el) => el.attribs['href']).toArray();
        const progress = progressBars.create(links?.length, 0, {month: currentMonth});

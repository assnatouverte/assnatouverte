...
import { proceedings, speeches, type NewSpeech } from "@assnatouverte/db/hansards";
...
async function saveSpeechToDatabase(db: Database, proceeding_id: string, order: number, speech: string[], speaker: string, speakerId?: string) {
    const speechString = speech.join("\n");
    ...
    //console.log(`%c${speaker} [${speakerId}]`, "color: blue;");
    ...
    await db.transaction(async tx => {
        ...
        for (const paragraph of paragraphs) {
            ...
            if (personIdentifier.length > 0 && paraType === ParagraphType.Text) {
                // Cleanup
                let personIdText = personIdentifier.map((_, el) => $(el).text()).get().join(' ');
                ...
                if (!(personIdText in speakers)) {
                    const lastNameMatch = personIdText.match(/(?:M\.|Mme)\s([\wà-üÀ-Ü\s\-]+)/);
                    if (lastNameMatch) {
                        const lastName = lastNameMatch[1]!;

                        // Query database
                        numRequests++;
                        const result = await db.select().from(members).innerJoin(membersToSessions, eq(members.id, membersToSessions.member_id)).where(and(eq(membersToSessions.legislature, metadata.legislature), eq(membersToSessions.session, metadata.session), like(members.last_name, lastName)));
                        ...
                        if (result.length == 1) {
                            ...
                            // Nobody found...
                            speakersNotFound[personIdText] = speakersNotFound[personIdText] + 1 || 1;
                        } else {
                            // Multiple possible members...
                            speakersAmbiguous[personIdText] = speakersAmbiguous[personIdText] + 1 || 1;
                        }
        ...
        // Delete old speech
        await tx.delete(speeches).where(and(eq(speeches.proceeding_id, metadata.id), gte(speeches.order, currentOrder)))
